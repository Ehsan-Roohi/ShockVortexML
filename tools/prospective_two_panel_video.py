...
def main():
    ...
    a.output.mkdir(parents=True,exist_ok=False); summary=[]
    for case_dir in sorted(p for p in a.inputs.iterdir() if p.is_dir()):
        frames=sorted(case_dir.glob("*.npz")); movie=a.output/case_dir.name; movie.mkdir(); scales=[]
        for src in frames:
        ...
        for i,src in enumerate(frames):
            ...
            target=movie/f"frame_{i:04d}.png"; fig.savefig(target,dpi=140); plt.close(fig)
